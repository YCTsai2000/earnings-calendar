...
class FetchSymbolEarningsTests(unittest.TestCase):
    ...
    def test_queries_one_symbol_and_filters_unrelated_rows(self, get):
        ...
        self.assertEqual(
            get.call_args.kwargs["params"]["symbol"],
            "MU",
        )
...
class ExistingCalendarFallbackTests(unittest.TestCase):
    ...
    def test_successful_empty_response_drops_future_event(self):
        existing = calendar.load_existing_events(
    ...
    def test_new_finnhub_date_replaces_old_future_date(self):
        existing = calendar.load_existing_events(
            str(self.ics_path), {"MU"}, START, END
        )
        new_item = {
        ...
        self.assertEqual(
            [item["date"] for item in fresh],
            ["2026-10-01"],
        )
        self.assertEqual(fresh[0]["_source_name"], "Finnhub")
        self.assertEqual(preserved, [])


class EventRenderingTests(unittest.TestCase):
    def test_all_events_use_one_finnhub_format(self):
        ...
        self.assertIn(
            "SUMMARY:TSLA 財報 (Q3 2026)",
            event,
        )
        self.assertNotIn("【預估】", event)
        self.assertNotIn("官方確認", event)
        self.assertNotIn("X-EARNINGS-SOURCE-STATUS", event)
        self.assertNotIn("STATUS:TENTATIVE", event)
        self.assertNotIn("STATUS:CONFIRMED", event)
